#ifndef SCOURGE__compiler__accounter
#define SCOURGE__compiler__accounter
...
typedef struct COMPILER__accountling_structure_member {
    ...
    SAILOR__bt predefined; // if the member is built in statically
...
void COMPILER__close__accountling_structure_member(COMPILER__accountling_structure_member member) {
    // close namespace
    if (member.predefined == SAILOR__bt__true) {
...
void COMPILER__append__accountling_structure_member(SAILOR__list* list, COMPILER__accountling_structure_member data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_structure_member), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_structure_member*)SAILOR__calculate__list_current_address(list)) = data;
...
typedef struct COMPILER__accountling_structure {
    ...
    SAILOR__counted_list members; // COMPILER__accountling_structure_member
    SAILOR__cell_count cell_count; // the count of all cells
    SAILOR__bt predefined;
} COMPILER__accountling_structure;

// close one structure
void COMPILER__close__accountling_structure(COMPILER__accountling_structure structure) {
    // close name
    if (structure.predefined == SAILOR__bt__true) {
        COMPILER__close__parsling_namespace(structure.name);
    }

    // set current
    SAILOR__current current_member = SAILOR__calculate__current_from_list_filled_index(&structure.members.list);

    // for each member
    while (SAILOR__check__current_within_range(current_member)) {
        // get member
    ...
    // close members
    SAILOR__close__counted_list(structure.members);

    return;
}

// append accountling structure
void COMPILER__append__accountling_structure(SAILOR__list* list, COMPILER__accountling_structure data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_structure), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_structure*)SAILOR__calculate__list_current_address(list)) = data;
...
typedef struct COMPILER__accountling_structures {
    SAILOR__counted_list name_table; // COMPILER__namespace
    SAILOR__counted_list data_table; // COMPILER__accountling_structure
...
void COMPILER__close__accountling_structures(COMPILER__accountling_structures structures) {
    ...
    SAILOR__current current_table_name = SAILOR__calculate__current_from_list_filled_index(&structures.name_table.list);
    COMPILER__structure_index name_index = 0;

    // for each name
    while (SAILOR__check__current_within_range(current_table_name) && name_index < COMPILER__ptt__USER_DEFINED_START) {
        // close name
    ...
    // close list
    SAILOR__close__counted_list(structures.name_table);

    // close structures
    // setup current
    SAILOR__current current_structure = SAILOR__calculate__current_from_list_filled_index(&structures.data_table.list);

    // for each structure
    while (SAILOR__check__current_within_range(current_structure)) {
        // get structure
    ...
    // close data table
    SAILOR__close__counted_list(structures.data_table);
...
typedef struct COMPILER__accountling_function_header {
    ...
    SAILOR__counted_list input_types; // COMPILER__structure_index
    SAILOR__counted_list output_types; // COMPILER__structure_index
} COMPILER__accountling_function_header;

// append accountling function header
void COMPILER__append__accountling_function_header(SAILOR__list* list, COMPILER__accountling_function_header data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_function_header), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_function_header*)SAILOR__calculate__list_current_address(list)) = data;
...
typedef struct COMPILER__accountling_function_headers {
    SAILOR__counted_list category[COMPILER__afht__COUNT]; // all headers categories, COMPILER__accountling_function_header, COMPILER__afht
...
COMPILER__accountling_function_headers COMPILER__open__accountling_function_headers(COMPILER__error* error) {
    ...
    // null init
    output.category[COMPILER__afht__predefined] = SAILOR__create_null__counted_list();
    output.category[COMPILER__afht__user_defined] = SAILOR__create_null__counted_list();
...
void COMPILER__close__accountling_function_headers__header_list(SAILOR__counted_list headers, SAILOR__bt close_namespaces) {
    // check for empty list
    if (SAILOR__check__empty_list(headers.list) == SAILOR__bt__true) {
    ...
    // close all headers
    // setup current
    SAILOR__current current_function_header = SAILOR__calculate__current_from_list_filled_index(&headers.list);

    // for each header
    while (SAILOR__check__current_within_range(current_function_header)) {
        // get header
        ...
        if (close_namespaces == SAILOR__bt__true) {
            // close name
            COMPILER__close__parsling_namespace(function_header.name);
        }

        // close io
        SAILOR__close__counted_list(function_header.input_types);
        SAILOR__close__counted_list(function_header.output_types);
    ...
    // close header buffer
    SAILOR__close__counted_list(headers);
...
void COMPILER__close__accountling_function_headers(COMPILER__accountling_function_headers headers) {
    // close lists
    COMPILER__close__accountling_function_headers__header_list(headers.category[COMPILER__afht__predefined], SAILOR__bt__true);
    COMPILER__close__accountling_function_headers__header_list(headers.category[COMPILER__afht__user_defined], SAILOR__bt__false);
...
// append accountling function header
void COMPILER__append__accountling_variable_argument(SAILOR__list* list, COMPILER__accountling_variable_argument data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_variable_argument), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_variable_argument*)SAILOR__calculate__list_current_address(list)) = data;
...
typedef struct COMPILER__accountling_scope {
    ...
    SAILOR__counted_list statements; // COMPILER__accountling_statement
...
typedef struct COMPILER__accountling_statement {
    ...
    // set cell data
    SAILOR__cell_integer_value set_cell__raw_value;
    ...
    // packer data
    SAILOR__counted_list pack__inputs; // COMPILER__accountling_variable_argument
    ...
    COMPILER__accountling_variable_argument compile__sailor_code;
    ...
    SAILOR__counted_list function_call__inputs; // COMPILER__accountling_variable_argument
    SAILOR__counted_list function_call__outputs; // COMPILER__accountling_variable_argument
...
COMPILER__accountling_statement COMPILER__create_null__accountling_statement() {
    ...
    output.scope_data.statements = SAILOR__create_null__counted_list();
    output.function_call__inputs = SAILOR__create_null__counted_list();
    output.function_call__outputs = SAILOR__create_null__counted_list();

    return output;
}

// append accountling statement
void COMPILER__append__accountling_statement(SAILOR__list* list, COMPILER__accountling_statement data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_statement), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_statement*)SAILOR__calculate__list_current_address(list)) = data;
...
typedef struct COMPILER__accountling_scope_header {
    ...
    SAILOR__offset starting_offset;
    SAILOR__offset ending_offset;
} COMPILER__accountling_scope_header;

// append accountling scope header
void COMPILER__append__accountling_scope_header(SAILOR__list* list, COMPILER__accountling_scope_header data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_scope_header), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_scope_header*)SAILOR__calculate__list_current_address(list)) = data;
...
void COMPILER__close__accountling_scope_headers(SAILOR__counted_list headers, COMPILER__error* error) {
    // search for scourge.master_scope
    // setup name
...
void COMPILER__append__accountling_variable(SAILOR__list* list, COMPILER__accountling_variable data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_variable), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_variable*)SAILOR__calculate__list_current_address(list)) = data;
...
typedef struct COMPILER__accountling_variables {
    SAILOR__counted_list lists[COMPILER__avat__COUNT]; // COMPILER__accountling_variable
...
void COMPILER__close__accountling_variables(COMPILER__accountling_variables variables) {
    ...
    for (COMPILER__avat index = COMPILER__avat__master; index < COMPILER__avat__COUNT; index++) {
        SAILOR__close__counted_list(variables.lists[index]);
    }
...
typedef struct COMPILER__accountling_function {
    // program data
    SAILOR__counted_list offsets; // COMPILER__namespace
    COMPILER__accountling_variables variables;
    SAILOR__counted_list scope_headers; // COMPILER__accountling_scope_header
    SAILOR__counted_list strings; // SAILOR__buffer
    COMPILER__accountling_scope scope;
    SAILOR__counted_list function_inputs; // COMPILER__accountling_variable_argument
    SAILOR__counted_list function_outputs; // COMPILER__accountling_variable_argument

    // statistics
    SAILOR__cell_count next_available_workspace_cell;
} COMPILER__accountling_function;

// append accountling function
void COMPILER__append__accountling_function(SAILOR__list* list, COMPILER__accountling_function data, COMPILER__error* error) {
    // request space
    SAILOR__list__request__space(list, sizeof(COMPILER__accountling_function), &(*error).memory_error_occured);

    // append data
    (*(COMPILER__accountling_function*)SAILOR__calculate__list_current_address(list)) = data;
...
void COMPILER__close__accountling_scope(COMPILER__accountling_scope scope) {
    ...
    for (COMPILER__statement_index index = 0; index < scope.statements.count; index++) {
        ...
        if (statement.statement_type == COMPILER__ast__scope) {
            ...
            SAILOR__close__counted_list(statement.pack__inputs);
        } else if (statement.statement_type == COMPILER__ast__user_defined_function_call) {
            // close io
            SAILOR__close__counted_list(statement.function_call__inputs);
            SAILOR__close__counted_list(statement.function_call__outputs);
        }
    }

    // close list
    SAILOR__close__counted_list(scope.statements);
    
    return;
}

// close strings
void COMPILER__close__accountling_strings(SAILOR__counted_list strings) {
    // close each individual string
    for (COMPILER__string_index index = 0; index < strings.count; index++) {
        // close string
        SAILOR__close__buffer(((SAILOR__buffer*)strings.list.buffer.start)[index]);
    }
    
    // close list
    SAILOR__close__counted_list(strings);
...
void COMPILER__close__accountling_function(COMPILER__accountling_function function, COMPILER__error* error) {
    // close offsets
    SAILOR__close__counted_list(function.offsets);
    ...
    // close function io
    SAILOR__close__counted_list(function.function_inputs);
    SAILOR__close__counted_list(function.function_outputs);
...
typedef struct COMPILER__accountling_functions {
    ...
    SAILOR__counted_list bodies; // COMPILER__accountling_function
...
void COMPILER__close__accountling_functions(COMPILER__accountling_functions functions, COMPILER__error* error) {
    ...
    // close list
    SAILOR__close__counted_list(functions.bodies);
...
COMPILER__structure_index COMPILER__find__accountling_structure_name_index(SAILOR__counted_list structure_names, COMPILER__namespace searching_for) {
    COMPILER__structure_index output = 0;

    // check each structure entry or name
    SAILOR__current current_structure = SAILOR__calculate__current_from_list_filled_index(&structure_names.list);
    ...
    while (SAILOR__check__current_within_range(current_structure)) {
        ...
        // check name
        if (COMPILER__check__identical_namespaces(searching_for, name) == SAILOR__bt__true) {
            // match!
            return output;
        }
...
COMPILER__structure_member_index COMPILER__find__accountling_structure_member_name_index(COMPILER__accountling_structure structure, COMPILER__namespace searching_for) {
    ...
    while (SAILOR__check__current_within_range(current_member)) {
        ...
        // check name
        if (COMPILER__check__identical_namespaces(searching_for, member.name) == SAILOR__bt__true) {
            // match!
            return output;
        }
...
SAILOR__counted_list COMPILER__account__structures__generate_predefined_type_names(SAILOR__counted_list names, COMPILER__error* error) {
    // loop through blueprint and generate structures
    // setup current
    SAILOR__current current_blueprintling = SAILOR__create__buffer((SAILOR__address)COMPILER__global__predefined_types, (SAILOR__address)(COMPILER__global__predefined_types + sizeof(COMPILER__global__predefined_types) - 1));

    // loop until end of blueprint
    while (SAILOR__check__current_within_range(current_blueprintling) && COMPILER__read__blueprintling(&current_blueprintling) == COMPILER__abt__define_type) {
...
SAILOR__counted_list COMPILER__account__structures__generate_predefined_type_data(SAILOR__counted_list structure_data, COMPILER__error* error) {
    // loop through blueprint and generate structures
    // setup current
    SAILOR__current current_blueprintling = SAILOR__create__buffer((SAILOR__address)COMPILER__global__predefined_types, (SAILOR__address)(COMPILER__global__predefined_types + sizeof(COMPILER__global__predefined_types) - 1));
    ...
    while (SAILOR__check__current_within_range(current_blueprintling) && COMPILER__read__blueprintling(&current_blueprintling) == COMPILER__abt__define_type) {
        ...
        accountling_structure.predefined = SAILOR__bt__true;
        ...
        accountling_structure.members = SAILOR__create__counted_list(COMPILER__open__list_with_error(sizeof(COMPILER__accountling_structure_member) * 16, error), 0);
        if (COMPILER__check__error_occured(error)) {
            return structure_data;
        }
        ...
        for (COMPILER__structure_member_index member_index = 0; member_index < member_count; member_index++) {
            ...
            member_temp.predefined = SAILOR__bt__true;
            if (COMPILER__check__error_occured(error)) {
                return structure_data;
            }
...
void COMPILER__account__check__recursive_structures__do_search(SAILOR__counted_list structures, COMPILER__stack_index current_stack_index, SAILOR__buffer type_trace, COMPILER__error* error) {
    // get structure ID
...
void COMPILER__account__check__recursive_structures(SAILOR__counted_list structures, COMPILER__error* error) {
    // setup tracker
    SAILOR__buffer type_trace = SAILOR__open__buffer(sizeof(COMPILER__structure_index) * structures.count);
    ...
    // destroy trace
    SAILOR__close__buffer(type_trace);

    return;
}

// search one structure for its cell count
void COMPILER__account__structures__calculate_structure_cell_counts__search(SAILOR__cell_count* cell_count, COMPILER__accountling_structures structures, COMPILER__structure_index searching, SAILOR__tab_count tabs) {
    // get structure
...
void COMPILER__account__structures__calculate_structure_cell_counts(COMPILER__accountling_structures structures) {
    ...
    for (COMPILER__structure_index index = 0; index < structures.data_table.count; index++) {
        // update structure
        // setup count
        SAILOR__cell_count cell_count = 0;
...
COMPILER__accountling_structures COMPILER__account__structures(COMPILER__parsling_program parsling_program, COMPILER__error* error) {
    ...
    // open tables
    output.name_table = SAILOR__create__counted_list(COMPILER__open__list_with_error(sizeof(COMPILER__namespace) * 32, error), 0);
    if (COMPILER__check__error_occured(error)) {
        return output;
    }
    output.data_table = SAILOR__create__counted_list(COMPILER__open__list_with_error(sizeof(COMPILER__accountling_structure) * 32, error), 0);
    if (COMPILER__check__error_occured(error)) {
        return output;
    }
    ...
    // get all names
    // for each structure
    SAILOR__current current_structure = SAILOR__calculate__current_from_list_filled_index(&parsling_program.structures.list);
    ...
    while (SAILOR__check__current_within_range(current_structure)) {
        ...
        SAILOR__current current_structure_name = SAILOR__calculate__current_from_list_filled_index(&structure.type_names.list);

        // for each structure name
        while (SAILOR__check__current_within_range(current_structure_name)) {
            // get name
    ...
    // for each structure
    current_structure = SAILOR__calculate__current_from_list_filled_index(&parsling_program.structures.list);
    ...
    while (SAILOR__check__current_within_range(current_structure)) {
        ...
        // for each structure name
        SAILOR__current current_structure_name = SAILOR__calculate__current_from_list_filled_index(&parsling_structure.type_names.list);
        ...
        while (SAILOR__check__current_within_range(current_structure_name)) {
            ...
            // setup accountling structure structure
            COMPILER__accountling_structure accountling_structure;
            accountling_structure.predefined = SAILOR__bt__false;
            ...
            SAILOR__current current_structure_member = SAILOR__calculate__current_from_list_filled_index(&parsling_structure.arguments.list);
            ...
            if (parsling_structure.arguments.count < 1) {
                ...
                *error = COMPILER__open__error("Accounting Error: A structure cannot have zero members.", COMPILER__get__namespace_lexling_location(structure_name.name));
                ...
                // close members
                SAILOR__close__counted_list(accountling_structure.members);

                return output;
            }

            // for each structure member
            while (SAILOR__check__current_within_range(current_structure_member)) {
                // get member
                ...
                accountling_member.predefined = SAILOR__bt__false;
...
SAILOR__bt COMPILER__account__search_for_function_header__check_headers_equal(COMPILER__accountling_function_header a, COMPILER__accountling_function_header b) {
    // check namespace
...
COMPILER__function_header_index COMPILER__account__search_for_function_header__search_list(SAILOR__counted_list headers_list, COMPILER__accountling_function_header searching_for) {
    // setup index
    COMPILER__function_header_index function_index = 0;

    // search each function header
    // setup current
    SAILOR__current current_header = SAILOR__calculate__current_from_list_filled_index(&headers_list.list);

    // for each header (unless found which quits early)
    while (SAILOR__check__current_within_range(current_header)) {
        // get header
        COMPILER__accountling_function_header header = *(COMPILER__accountling_function_header*)current_header.start;

        // check for match
        if (COMPILER__account__search_for_function_header__check_headers_equal(header, searching_for) == SAILOR__bt__true) {
            // match!
...
COMPILER__accountling_function_headers COMPILER__account__functions__generate_predefined_function_headers(COMPILER__accountling_structures structures, COMPILER__accountling_function_headers headers, COMPILER__error* error) {
    ...
    SAILOR__current current_blueprintling = SAILOR__create__buffer((SAILOR__address)COMPILER__global__predefined_one_off_function_calls, (SAILOR__address)COMPILER__global__predefined_one_off_function_calls + sizeof(COMPILER__global__predefined_one_off_function_calls) - 1);

    // for each definition
    while (SAILOR__check__current_within_range(current_blueprintling) && COMPILER__read__blueprintling(&current_blueprintling) == COMPILER__abt__define_function_call) {
        // skip past marker
        COMPILER__next__blueprintling(&current_blueprintling);
        ...
        // allocate inputs
        header.input_types = SAILOR__create__counted_list(COMPILER__open__list_with_error(sizeof(COMPILER__structure_index) * 8, error), 0);
        ...
        // allocate outputs
        header.output_types = SAILOR__create__counted_list(COMPILER__open__list_with_error(sizeof(COMPILER__structure_index) * 8, error), 0);
    ...
    for (COMPILER__structure_index index = 0; index < structures.name_table.count; index++) {
        ...
        // allocate io
        header.input_types = SAILOR__open__counted_list(sizeof(COMPILER__structure_index) * 1, &(*error).memory_error_occured);
        if (COMPILER__check__error_occured(error)) {
            return headers;
        }
        header.output_types = SAILOR__open__counted_list(sizeof(COMPILER__structure_index) * 1, &(*error).memory_error_occured);
        if (COMPILER__check__error_occured(error)) {
            return headers;
        }
    ...
    // generate packer headers (skip scourge.cell)
    for (COMPILER__structure_index structure_index = 1; structure_index < structures.data_table.count; structure_index++) {
        ...
        // setup inputs
        // allocate inputs
        header.input_types = SAILOR__open__counted_list(sizeof(COMPILER__structure_index) * ((COMPILER__accountling_structure*)structures.data_table.list.buffer.start)[structure_index].members.count, &(*error).memory_error_occured);
        if (COMPILER__check__error_occured(error)) {
            return headers;
        }
    ...
    for (COMPILER__structure_index structure_index = 0; structure_index < structures.data_table.count; structure_index++) {
        ...
        // append structure
        COMPILER__append__structure_index(&header.output_types.list, COMPILER__aat__COUNT + COMPILER__ptt__scourge_cell, error);
        if (COMPILER__check__error_occured(error)) {
            return headers;
        }
        header.output_types.count = 1;
    ...
    for (COMPILER__structure_index structure_index = 0; structure_index < structures.data_table.count; structure_index++) {
        ...
        // append structure
        COMPILER__append__structure_index(&header.input_types.list, COMPILER__aat__COUNT + COMPILER__ptt__scourge_buffer, error);
        if (COMPILER__check__error_occured(error)) {
            return headers;
        }
        header.input_types.count = 2;
        ...
        // append structure
        COMPILER__append__structure_index(&header.input_types.list, COMPILER__aat__COUNT + COMPILER__ptt__scourge_cell, error);
        if (COMPILER__check__error_occured(error)) {
            return headers;
        }
        header.output_types.count = 1;
...
SAILOR__counted_list COMPILER__account__functions__user_defined_function_header_arguments(SAILOR__counted_list structure_names, SAILOR__counted_list parsling_header_arguments, COMPILER__error* error) {
    // allocate structure index list
    SAILOR__counted_list output = COMPILER__open__counted_list_with_error(sizeof(COMPILER__structure_index) * 8, error);

    // setup current
    SAILOR__current current_parsling_argument = SAILOR__calculate__current_from_list_filled_index(&parsling_header_arguments.list);

    // for each argument
    while (SAILOR__check__current_within_range(current_parsling_argument)) {
        // get argument
...
COMPILER__accountling_function_headers COMPILER__account__functions__user_defined_function_headers(COMPILER__accountling_function_headers headers, COMPILER__parsling_program parsling_program, SAILOR__counted_list structure_names, COMPILER__error* error) {
    // for each function
    // setup current
    SAILOR__current current_function = SAILOR__calculate__current_from_list_filled_index(&parsling_program.functions.list);

    // for each function
    while (SAILOR__check__current_within_range(current_function)) {
...
COMPILER__scope_index COMPILER__account__functions__get_scope_index(COMPILER__accountling_function* accountling_function, COMPILER__namespace scope_name) {
    ...
    while (output < (*accountling_function).scope_headers.count) {
        // check name
        if (COMPILER__check__identical_namespaces(((COMPILER__accountling_scope_header*)(*accountling_function).scope_headers.list.buffer.start)[output].name, scope_name) == SAILOR__bt__true) {
            // match!
            return output;
        }
...
COMPILER__offset_index COMPILER__account__functions__get_offset_index(COMPILER__accountling_function* accountling_function, COMPILER__namespace offset_name) {
    ...
    while (output < (*accountling_function).offsets.count) {
        // check name
        if (COMPILER__check__identical_namespaces(((COMPILER__namespace*)(*accountling_function).offsets.list.buffer.start)[output], offset_name) == SAILOR__bt__true) {
            // match!
            return output;
        }
...
void COMPILER__account__functions__get_scope_level_data(COMPILER__accountling_function* accountling_function, COMPILER__parsling_scope parsling_scope, COMPILER__error* error) {
    ...
    // setup current
    SAILOR__current current_statement = SAILOR__calculate__current_from_list_filled_index(&parsling_scope.statements.list);

    // for each statement
    while (SAILOR__check__current_within_range(current_statement)) {
        // get statement
        ...
        if (parsling_statement.type == COMPILER__stt__subscope) {
            ...
            scope_header.starting_offset = SAILOR__invalid_offset;
            scope_header.ending_offset = SAILOR__invalid_offset;
...
SAILOR__buffer COMPILER__translate__string_literal_to_translated_buffer(COMPILER__lexling string_literal, COMPILER__error* error) {
    SAILOR__buffer output = SAILOR__create_null__buffer();
    SAILOR__length character_count = 0;

    // count characters
    for (SAILOR__character_index i = 1; i < SAILOR__calculate__buffer_length(string_literal.value) - 1; i++) {
        // check for escape sequence
        if (((SAILOR__character*)string_literal.value.start)[i] == '%') {
            // check for characters available
            if (i + 3 <= SAILOR__calculate__buffer_length(string_literal.value) - 1) {
                // validate characters
                SAILOR__bt invalid_hexadecimal_character_1;
                SAILOR__bt invalid_hexadecimal_character_2;
                SAILOR__bt semi_colon_missing;
                COMPILER__translate__character_to_hexadecimal(((SAILOR__character*)string_literal.value.start)[i + 1], &invalid_hexadecimal_character_1);
                COMPILER__translate__character_to_hexadecimal(((SAILOR__character*)string_literal.value.start)[i + 2], &invalid_hexadecimal_character_2);
                semi_colon_missing = (SAILOR__bt)(((SAILOR__character*)string_literal.value.start)[i + 3] != ';');

                // determine validity
                if (invalid_hexadecimal_character_1 != SAILOR__bt__false || invalid_hexadecimal_character_2 != SAILOR__bt__false || semi_colon_missing != SAILOR__bt__false) {
    ...
    // allocate string
    output = SAILOR__open__buffer(character_count);
    if (SAILOR__check__empty_buffer(output)) {
    ...
    for (SAILOR__character_index i = 1; i < SAILOR__calculate__buffer_length(string_literal.value) - 1; i++) {
        ...
        if (((SAILOR__character*)string_literal.value.start)[i] == '%') {
            ...
            SAILOR__bt invalid_hexadecimal_character_1;
            SAILOR__bt invalid_hexadecimal_character_2;
            SAILOR__character a = COMPILER__translate__character_to_hexadecimal(((SAILOR__character*)string_literal.value.start)[i + 1], &invalid_hexadecimal_character_1);
            SAILOR__character b = COMPILER__translate__character_to_hexadecimal(((SAILOR__character*)string_literal.value.start)[i + 2], &invalid_hexadecimal_character_2);

            // write character
            ((SAILOR__character*)output.start)[character_count] = b + (a << 4);
            
            // skip past characters
            i += 3;
        } else {
            // write character
            ((SAILOR__character*)output.start)[character_count] = ((SAILOR__character*)string_literal.value.start)[i];
        }
...
SAILOR__bt COMPILER__check__accountling_variable_argument_is_declared(COMPILER__accountling_variables variables, COMPILER__accountling_variable_argument index) {
...
SAILOR__bt COMPILER__account__functions__get_variable_by_variable_argument__recursively(COMPILER__accountling_variables variables, COMPILER__accountling_variable_argument variable_argument, COMPILER__namespace name, COMPILER__lexling_index sub_name_index, COMPILER__accountling_variable_argument* output) {
    // setup variable temp
    ...
    for (COMPILER__variable_member_index member_index = variable.members.start; member_index <= variable.members.end; member_index++) {
        ...
        // if names match
        if (SAILOR__calculate__buffer_contents_equal(member.name.value, COMPILER__get__lexling_by_index(name.lexlings, sub_name_index).value) == SAILOR__bt__true) {
            // name matches
...
COMPILER__accountling_variable_argument COMPILER__account__functions__get_variable_argument_by_name(COMPILER__accountling_variables variables, COMPILER__namespace name) {
    ...
    for (COMPILER__variable_index master_index = 0; master_index < variables.lists[COMPILER__avat__master].count; master_index++) {
        ...
        // check master names
        if (SAILOR__calculate__buffer_contents_equal(variable.name.value, COMPILER__get__lexling_by_index(name.lexlings, 0).value)) {
            ...
            // variable name is longer than just master
                ...
                // get variable
                if (COMPILER__account__functions__get_variable_by_variable_argument__recursively(variables, COMPILER__create__accountling_variable_argument(COMPILER__avat__master, master_index), name, 1, &output) == SAILOR__bt__true) {
                    return output;
                }
            }
        }
...
COMPILER__accountling_variable_range COMPILER__account__functions__mark_variable__generate_structure_variable_members(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, SAILOR__cell_ID* current_sub_cell, COMPILER__variable_type_index type, COMPILER__error* error) {
    COMPILER__accountling_variable_range output;
    ...
    for (COMPILER__structure_member_index index = output.start; index <= output.end; index++) {
        ...
        // recurse
            ...
            // recurse
            if (member.structure_ID > COMPILER__ptt__scourge_cell) {
...
void COMPILER__account__functions__mark_variable__generate_master_variable_and_sub_variables(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_argument name, COMPILER__variable_type_index master_type, COMPILER__error* error) {
    ...
    COMPILER__accountling_variable_range member_range;
    if (master_type < structures.data_table.count && master_type > COMPILER__ptt__scourge_cell) {
...
COMPILER__accountling_variable_argument COMPILER__account__functions__mark_variable(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_argument name, COMPILER__variable_type_index expected_type, COMPILER__asvt argument_location, SAILOR__bt declare_if_nonexistant, SAILOR__bt* is_valid_argument, COMPILER__error* error) {
    ...
    if (COMPILER__check__empty_namespace(name.type) == SAILOR__bt__false && COMPILER__find__accountling_structure_name_index(structures.name_table, name.type) != expected_type) {
        // not a match
        goto failure;
    }
    ...
    if (name.name.lexlings.count < 2) {
        ...
        // is already declared
        if (COMPILER__check__accountling_variable_argument_is_declared((*accountling_function).variables, argument) == SAILOR__bt__true) {
        ...
        // is not declared
            ...
            if (argument_location == COMPILER__asvt__output) {
                ...
                if (declare_if_nonexistant == SAILOR__bt__true) {
            ...
            if ((*accountling_function).next_available_workspace_cell >= SAILOR__srt__start__function_io) {
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__sets(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__set], parsling_statement.name.name) && parsling_statement.inputs.count == 1 && parsling_statement.outputs.count == 1) {
        ...
        if (COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0).category == COMPILER__pat__string_literal && COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0).category == COMPILER__pat__name) {
            ...
            // get index
            SAILOR__bt is_valid_argument;
            COMPILER__accountling_variable_argument variable_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || variable_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }

            // convert string
            SAILOR__buffer string_data = COMPILER__translate__string_literal_to_translated_buffer(COMPILER__get__lexling_by_index(COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0).name.lexlings, 0), error);
            if (COMPILER__check__error_occured(error) && SAILOR__check__empty_buffer(string_data) == SAILOR__bt__false) {
                // close string
                SAILOR__close__buffer(string_data);
                goto failure;
            }
        ...
        // check for integer literal
            ...
            // temporaries
            SAILOR__cell_integer_value integer_value = 0;
            SAILOR__buffer integer_string = COMPILER__get__lexling_by_index(COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0).name.lexlings, 0).value;
            ...
            if (COMPILER__translate__string_to_boolean(integer_string, &integer_value) || COMPILER__translate__string_to_binary(integer_string, &integer_value) || COMPILER__translate__string_to_integer(integer_string, &integer_value) || COMPILER__translate__string_to_hexadecimal(integer_string, &integer_value)) {
                // get index
                SAILOR__bt is_valid_argument;
                COMPILER__accountling_variable_argument variable_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
                if (COMPILER__check__error_occured(error) || variable_argument.type >= COMPILER__avat__COUNT) {
                    goto failure;
                }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__prints(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__print__character], parsling_statement.name.name) && parsling_statement.inputs.count == 1 && parsling_statement.outputs.count == 0) {
        ...
        if (COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0).category == COMPILER__pat__name) {
            ...
            // get index
            SAILOR__bt is_valid_argument;
            COMPILER__accountling_variable_argument variable_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || variable_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
...
// check for packers
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__copy(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    // if is a copy
...
// check for packers
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__packers(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    // if is a packer
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__pack], parsling_statement.name.name) && parsling_statement.outputs.count == 1) {
        ...
        if (COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0).category == COMPILER__pat__name && packing_type < structures.name_table.count) {
            ...
            // get index
            SAILOR__bt is_valid_argument;
            COMPILER__accountling_variable_argument variable_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), packing_type, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || variable_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            ...
            if (packing_structure.members.count == parsling_statement.inputs.count) {
                ...
                for (COMPILER__structure_member_index index = 0; index < packing_structure.members.count; index++) {
                    ...
                    COMPILER__accountling_variable_argument argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, index), accountling_member.structure_ID, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
                    if (COMPILER__check__error_occured(error)) {
                        goto failure;
                    }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__null_packers(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    // if is a packer
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__increment_packers(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    // if is a packer
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__pack_increment], parsling_statement.name.name) && parsling_statement.inputs.count == 2 && parsling_statement.outputs.count == 1) {
        ...
        if (COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0).category == COMPILER__pat__name && COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1).category == COMPILER__pat__name && COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0).category == COMPILER__pat__name && packing_type < structures.name_table.count) {
            // mark variables
            SAILOR__bt is_valid_argument;
            COMPILER__accountling_variable_argument increment_start_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || increment_start_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            COMPILER__accountling_variable_argument increment_by_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || increment_by_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            COMPILER__accountling_variable_argument output_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), packing_type, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || output_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__jumping(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    // if is a top jump
...
// check for checking
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__data_checking(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__integer__within_range], parsling_statement.name.name) && parsling_statement.inputs.count == 3 && parsling_statement.outputs.count == 2) {
        ...
        if (COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0).category == COMPILER__pat__name && COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1).category == COMPILER__pat__name && COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 2).category == COMPILER__pat__name && COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0).category == COMPILER__pat__name && COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 1).category == COMPILER__pat__name) {
            ...
            // get index
            SAILOR__bt is_valid_argument;
            COMPILER__accountling_variable_argument lower_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || lower_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            COMPILER__accountling_variable_argument checking_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || checking_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            COMPILER__accountling_variable_argument higher_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 2), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || higher_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            COMPILER__accountling_variable_argument result_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || result_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
            COMPILER__accountling_variable_argument inverted_result_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__integer_operations(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (parsling_statement.inputs.count == 2 && parsling_statement.outputs.count == 1 && (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__integer__add], parsling_statement.name.name) || COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__integer__subtract], parsling_statement.name.name) || COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__integer__multiply], parsling_statement.name.name) || COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__integer__divide], parsling_statement.name.name) || COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__integer__modulous], parsling_statement.name.name))) {
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument first_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || first_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument second_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || second_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument output_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || output_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__bit_operations(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    SAILOR__bt is_valid_argument;
    ...
    // if is an overwrite
        ...
        COMPILER__accountling_variable_argument third_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 2), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__buffer_acquisition(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__buffer__request], parsling_statement.name.name) && parsling_statement.inputs.count == 1 && parsling_statement.outputs.count == 1) {
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument input_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || input_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument output_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || output_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
    ...
    // if is buffer return
        ...
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument input_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || input_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__cell_address_movers(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__buffer__address_to_cell], parsling_statement.name.name) && parsling_statement.inputs.count == 2 && parsling_statement.outputs.count == 2) {
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument address_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || address_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument byte_count_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || byte_count_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument cell_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || cell_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument advancement_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
    ...
    // if is cell to address
        ...
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument cell_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || cell_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument byte_count_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        ...
        COMPILER__accountling_variable_argument address_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 2), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || address_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument advancement_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || advancement_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__file_buffer_movers(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__buffer__file_to_buffer], parsling_statement.name.name) && parsling_statement.inputs.count == 1 && parsling_statement.outputs.count == 1) {
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument file_path_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || file_path_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument buffer_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
    ...
    // if is buffer to file
        ...
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument buffer_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || buffer_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument file_path_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || file_path_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
    ...
    // if is buffer copy low to high
        ...
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument first_buffer_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || first_buffer_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument second_buffer_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__structure_buffer_movers(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__structure__structure_byte_size], parsling_statement.name.name) && parsling_statement.inputs.count == 1 && parsling_statement.outputs.count == 1) {
        ...
        // get variables
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument structure_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__account__functions__get_variable_by_variable_argument((*accountling_function).variables, structure_type).type, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || structure_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument size_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
    ...
    // if is structure to buffer
        ...
        COMPILER__accountling_variable_argument buffer_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || buffer_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument advancement_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
    ...
    // if is buffer to structure
        ...
        if (COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0).category == COMPILER__pat__name && structure_type < structures.name_table.count) {
            ...
            COMPILER__accountling_variable_argument structure_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), structure_type, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
            if (COMPILER__check__error_occured(error) || structure_argument.type >= COMPILER__avat__COUNT) {
                goto failure;
            }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__compiler_assorted(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__compiler__compile], parsling_statement.name.name) && parsling_statement.inputs.count == 2 && parsling_statement.outputs.count == 3) {
        ...
        COMPILER__accountling_variable_argument settings_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_compiler__settings, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || settings_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument sailor_code_output_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || sailor_code_output_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument debug_information_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 1), COMPILER__ptt__scourge_compiler__debug_information, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || debug_information_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument error_information_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 2), COMPILER__ptt__scourge_compiler__error, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        ...
        (*accountling_statement).compile__sailor_code = sailor_code_output_argument;
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__context_assorted(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__context__install_program], parsling_statement.name.name) && parsling_statement.inputs.count == 2 && parsling_statement.outputs.count == 0) {
        ...
        // get index
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument context_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || context_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument program_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
    ...
    // if is a context run call
        ...
        COMPILER__accountling_variable_argument instruction_count_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 1), COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
...
// check for time
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__time_functions(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__time__now], parsling_statement.name.name) && parsling_statement.inputs.count == 0 && parsling_statement.outputs.count == 1) {
        ...
        // get index
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument output_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_time, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || output_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__allocation_related_functions(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
    ...
    if (COMPILER__check__namespace_against_c_string(COMPILER__global__predefined_function_call_names[COMPILER__pfcnt__buffer__find_allocation], parsling_statement.name.name) && parsling_statement.inputs.count == 1 && parsling_statement.outputs.count == 2) {
        ...
        // get index
        SAILOR__bt is_valid_argument;
        COMPILER__accountling_variable_argument source_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.inputs, 0), COMPILER__ptt__scourge_buffer, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || source_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument was_found_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 0), COMPILER__ptt__scourge_cell, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error) || was_found_argument.type >= COMPILER__avat__COUNT) {
            goto failure;
        }
        COMPILER__accountling_variable_argument found_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, COMPILER__get__parsling_argument_by_index(parsling_statement.outputs, 1), COMPILER__ptt__scourge_buffer, COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
...
SAILOR__bt COMPILER__account__functions__check_and_get_statement_translation__user_defined_function_calls(COMPILER__accountling_structures structures, COMPILER__accountling_function_headers function_headers, COMPILER__accountling_function* accountling_function, COMPILER__parsling_statement parsling_statement, COMPILER__accountling_statement* accountling_statement, COMPILER__error* error) {
...
void COMPILER__account__functions__function_sequential_information__one_scope(COMPILER__accountling_structures structures, COMPILER__accountling_function_headers function_headers, COMPILER__accountling_function* accountling_function, COMPILER__accountling_scope* accountling_scope, COMPILER__parsling_statement parsling_source_statement, COMPILER__error* error) {
    ...
    SAILOR__current current_statement = SAILOR__calculate__current_from_list_filled_index(&parsling_source_statement.subscope.statements.list);
    ...
    if (COMPILER__check__empty_namespace(parsling_source_statement.name.name) == SAILOR__bt__false) {
        SAILOR__bt is_valid_argument;
        (*accountling_scope).condition = COMPILER__account__functions__mark_variable(structures, accountling_function, parsling_source_statement.subscope_flag_name, COMPILER__ptt__scourge_cell, COMPILER__asvt__input, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__check__error_occured(error)) {
            return;
        }
        if (is_valid_argument == SAILOR__bt__false) {
    ...
    // otherwise, a master scope requires a manual condition
        ...
        // setup name
        COMPILER__namespace temp_search_name = COMPILER__open__namespace_from_single_lexling(COMPILER__create__lexling(COMPILER__lt__name, SAILOR__open__buffer_from_string((u8*)(COMPILER__define__master_namespace ".always"), SAILOR__bt__false, SAILOR__bt__false), COMPILER__create_null__character_location()), error);
        if (COMPILER__check__error_occured(error)) {
            return;
        }
...
COMPILER__variable_index COMPILER__account__functions__function_io_variables__get_argument_index(SAILOR__counted_list variable_arguments, COMPILER__accountling_variable_argument argument) {
...
void COMPILER__account__functions__function_io_variables(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__parsling_function parsling_function, COMPILER__error* error) {
    ...
    for (COMPILER__argument_index io_index = 0; io_index < parsling_function.header.inputs.count; io_index++) {
        ...
        COMPILER__accountling_variable_argument accountling_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, parsling_argument, COMPILER__find__accountling_structure_name_index(structures.name_table, parsling_argument.type), COMPILER__asvt__output, SAILOR__bt__false, &is_valid_argument, error);
        if (accountling_argument.index >= COMPILER__avat__COUNT && is_valid_argument == SAILOR__bt__true) {
            // declare variable
            accountling_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, parsling_argument, COMPILER__find__accountling_structure_name_index(structures.name_table, parsling_argument.type), COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
    ...
    for (COMPILER__argument_index io_index = 0; io_index < parsling_function.header.outputs.count; io_index++) {
        ...
        // check if variable exists
        COMPILER__accountling_variable_argument accountling_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, parsling_argument, COMPILER__find__accountling_structure_name_index(structures.name_table, parsling_argument.type), COMPILER__asvt__output, SAILOR__bt__false, &is_valid_argument, error);
        if (COMPILER__account__functions__function_io_variables__get_argument_index((*accountling_function).function_outputs, accountling_argument) >= (*accountling_function).function_outputs.count && is_valid_argument == SAILOR__bt__true) {
            // declare variable
            accountling_argument = COMPILER__account__functions__mark_variable(structures, accountling_function, parsling_argument, COMPILER__find__accountling_structure_name_index(structures.name_table, parsling_argument.type), COMPILER__asvt__output, SAILOR__bt__true, &is_valid_argument, error);
...
void COMPILER__account__functions__predefined_variables(COMPILER__accountling_structures structures, COMPILER__accountling_function* accountling_function, COMPILER__error* error) {
    ...
    // setup current
    SAILOR__current current_blueprintling = SAILOR__create__buffer((SAILOR__address)COMPILER__global__predefined_variables, (SAILOR__address)(COMPILER__global__predefined_variables + sizeof(COMPILER__global__predefined_variables) - 1));
    ...
    while (SAILOR__check__current_within_range(current_blueprintling) && COMPILER__read__blueprintling(&current_blueprintling) == COMPILER__abt__define_type) {
        ...
        variable.name = COMPILER__create__lexling(COMPILER__lt__name, SAILOR__open__buffer_from_string((u8*)COMPILER__global__predefined_variable_names[(COMPILER__index)COMPILER__read_and_next__blueprintling(&current_blueprintling)], SAILOR__bt__false, SAILOR__bt__false), COMPILER__create_null__character_location());

        // read type
        variable.type = (COMPILER__variable_type_index)COMPILER__read_and_next__blueprintling(&current_blueprintling);

        // get cell range
        variable.cells.start = (SAILOR__cell_ID)COMPILER__read_and_next__blueprintling(&current_blueprintling);
        variable.cells.end = (SAILOR__cell_ID)COMPILER__read_and_next__blueprintling(&current_blueprintling);
...
COMPILER__accountling_functions COMPILER__account__functions__user_defined_function_bodies(COMPILER__accountling_functions functions, COMPILER__accountling_structures structures, COMPILER__parsling_program parsling_program, COMPILER__error* error) {
    ...
    SAILOR__current current_parsling_function = SAILOR__calculate__current_from_list_filled_index(&parsling_program.functions.list);

    // for each function
    while (SAILOR__check__current_within_range(current_parsling_function)) {
        // get function
        ...
        // account function
        {
            ...
            accountling_function.next_available_workspace_cell = SAILOR__srt__start__workspace;
            ...
            if (COMPILER__check__error_occured(error)) {
                // close data
                SAILOR__close__counted_list(accountling_function.offsets);
                COMPILER__close__accountling_scope_headers(accountling_function.scope_headers, error);
            ...
            // open strings
            accountling_function.strings = COMPILER__open__counted_list_with_error(sizeof(SAILOR__buffer) * 16, error);
            if (COMPILER__check__error_occured(error)) {
                // close prior data
                SAILOR__close__counted_list(accountling_function.offsets);
                COMPILER__close__accountling_scope_headers(accountling_function.scope_headers, error);
            ...
            if (COMPILER__check__error_occured(error)) {
                ...
                COMPILER__close__accountling_scope_headers(accountling_function.scope_headers, error);
                COMPILER__close__accountling_variables(accountling_function.variables);
                SAILOR__close__counted_list(accountling_function.function_inputs);
                SAILOR__close__counted_list(accountling_function.function_outputs);
...
COMPILER__function_index COMPILER__find__entry_point(COMPILER__accountling_functions functions, COMPILER__error* error) {
    ...
    for (COMPILER__function_index index = 0; index < functions.headers.category[COMPILER__afht__user_defined].count; index++) {
        ...
        // check header
        if (COMPILER__check__identical_namespaces(header.name, searching_for) == SAILOR__bt__true && header.input_types.count == 0 && header.output_types.count == 0) {
            // match!
...
void COMPILER__print__accountling_structure(COMPILER__accountling_structures accountling_structures, COMPILER__accountling_structure accountling_structure, COMPILER__structure_index index, SAILOR__tab_count tab_depth) {
    // print header
    SAILOR__print__tabs(tab_depth);
    ...
    // print members
    // setup current
    SAILOR__current current_member = SAILOR__calculate__current_from_list_filled_index(&accountling_structure.members.list);
    ...
    while (SAILOR__check__current_within_range(current_member)) {
        ...
        // print header
        SAILOR__print__tabs(tab_depth + 1);
...
void COMPILER__print__accountling_structures(COMPILER__accountling_structures structures, SAILOR__tab_count tab_depth) {
    // print header
    SAILOR__print__tabs(tab_depth);
    ...
    // print name table
    // setup current
    SAILOR__current current_name = SAILOR__calculate__current_from_list_filled_index(&structures.name_table.list);

    // for each name
    while (SAILOR__check__current_within_range(current_name)) {
        // get name
...
void COMPILER__print__accountling_function_header_arguments(SAILOR__counted_list arguments) {
    // print start of arguments
    printf("(");

    // setup current
    SAILOR__current current_structure_index = SAILOR__calculate__current_from_list_filled_index(&arguments.list);

    // for each structure index
    while (SAILOR__check__current_within_range(current_structure_index)) {
        // get structure index
    ...
    if (variable.members.start != (COMPILER__variable_member_index)-1) {
        for (COMPILER__variable_member_index index = variable.members.start; index <= variable.members.end; index++) {
            ...
            if (member_variable.type < structures.data_table.count) {
                // print member
                COMPILER__print__accountling_variable(function, structures, member_variable, member_index, SAILOR__bt__true, master_variable_index, tab_depth + 1);
            }
        }
    }
...
void COMPILER__print__accountling_scope(COMPILER__accountling_scope statements, SAILOR__tab_count tab_depth) {
...
void COMPILER__print__accountling_functions(COMPILER__accountling_structures structures, COMPILER__accountling_functions functions, SAILOR__tab_count tab_depth) {
    ...
    for (COMPILER__afht category_index = 0; category_index < COMPILER__afht__COUNT; category_index++) {
        ...
        // print header table
        // setup current
        SAILOR__current current_header = SAILOR__calculate__current_from_list_filled_index(&functions.headers.category[category_index].list);
        ...
        while (SAILOR__check__current_within_range(current_header)) {
            ...
            // print header
            SAILOR__print__tabs(tab_depth + 2);
            ...
            // print data
            {
                ...
                if (function.scope_headers.count > 0) {
                    ...
                    for (COMPILER__scope_index scope_index = 0; scope_index < function.scope_headers.count; scope_index++) {
                        ...
                        // print scope data
                        SAILOR__print__tabs(tab_depth + 3);
                ...
                if (function.variables.lists[COMPILER__avat__master].count > 0) {
                    ...
                    // print header
                    SAILOR__print__tabs(tab_depth + 2);
                    printf("Variables [ cells_used: %lu/%lu (%lu->%lu) ]:\n", function.next_available_workspace_cell - (SAILOR__u64)SAILOR__srt__start__workspace, (SAILOR__u64)SAILOR__srt__workspace__COUNT, (SAILOR__u64)SAILOR__srt__start__workspace, (SAILOR__u64)function.next_available_workspace_cell - 1);
                    ...
                    for (COMPILER__variable_index index = 0; index < function.variables.lists[COMPILER__avat__master].count; index++) {
                        ...
                        COMPILER__print__accountling_variable(function, structures, master_variable, &member_index, SAILOR__bt__false, index, tab_depth + 3);
                    }
                }
                ...
                if (function.strings.count > 0) {
                    ...
                    for (COMPILER__string_index string_index = 0; string_index < function.strings.count; string_index++) {
                        // get string
                        SAILOR__buffer string = ((SAILOR__buffer*)function.strings.list.buffer.start)[string_index];

                        // print string data
                        SAILOR__print__tabs(tab_depth + 3);
                        printf("[ %lu ] \"", string_index);
                        SAILOR__print__buffer(string);
                        printf("\"\n");
                    }
                }
            }
